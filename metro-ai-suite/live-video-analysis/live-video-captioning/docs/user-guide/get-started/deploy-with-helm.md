...
- An Intel GPU/NPU-capable worker node is recommended so the `metrics-service` can report GPU and NPU utilization (via qmassa). GPU/NPU metrics are simply omitted on nodes without the hardware.
...
- If the `metrics-service` does not report metrics, confirm that the pod is scheduled on the selected node (it binds host ports `9090` and `9273`) and that it has the required privileged/hostPID access for GPU and NPU collection.
