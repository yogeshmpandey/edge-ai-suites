...
| Industrial / logistics | surface-defect, PPE compliance, zone intrusion, pallet/forklift tracking |
...
`{{PIPELINE_NAME}}`, `{{DEFAULT_MODEL}}`, `{{DEFAULT_RULE}}`, and
...
| [`references/SCENESCAPE.md`](references/SCENESCAPE.md) | **Only when `{{SCENESCAPE}}=yes`** — opt-in multi-camera scene-fusion path, delegating to the `scenescape-setup` skill |
| [`references/DEMO_POC.md`](references/DEMO_POC.md) | **Only when `{{MODE}}=demo`** — lightweight single-app path (DL Streamer or OpenVINO); no full stack |

## Parameters (from invoking prompt)

| Param | Purpose |
|---|---|
| `{{MODE}}` | `demo` \| `production` (default `production`). `demo` = lightweight single-app path ([`references/DEMO_POC.md`](references/DEMO_POC.md)); params below apply to `production` only |
| `{{OBJECT}}` | class label in dashboard/alerts (e.g. `person`, `vehicle`, `hardhat`, `defect`, `fall`) — any MQTT/Grafana-safe string |
...
| `{{SCENESCAPE}}` | `yes` \| `no` (default `no`). `yes` = opt-in multi-camera spatial-analysis path ([`references/SCENESCAPE.md`](references/SCENESCAPE.md)) |
...
[Demo/PoC mode](#demopoc-mode); do **not** ask questions 1–7. Questions 1–7
...
failure. Both the script body and the full **validation rules table**
(`MODE`, `HOST_IP`, `NUM_SOURCES`, `DEVICE`, `PIPELINE_NAME`, topics, TURN
...
`/<pid>/whep|whip`→WHEP/WHIP signalling, `/webrtc/`→MediaMTX local TCP (ICE 8189).
Data: DLSPS→MQTT→Mosquitto→Node-RED→Grafana (mqtt datasource); DLSPS→WHIP→MediaMTX
(peer-id `{{DETECTIONS_TOPIC_PREFIX}}_N`, ICE/TURN via Coturn); Grafana embeds
...
When Question 7 selects SceneScape, **branch**: keep the DLSPS detection
pipeline but replace the MediaMTX/WebRTC + Node-RED + Grafana-MQTT tail with an
Intel® SceneScape multi-camera scene-fusion stack. **Do not re-implement by
hand** — delegate to the `scenescape-setup` skill, passing `{{SCENE_NAME}}` and
`{{CAMERA_IDS}}`. Architecture, images, validation, and completion criteria are
in [`references/SCENESCAPE.md`](references/SCENESCAPE.md); load only on this
...
the repo's `tags` API with `ordering=last_updated`), then pin that concrete
version — never floating `:latest`, and ignore `*-weekly` pre-releases.
**Grafana is the one exception:** keep it pinned to `11.5.4` (the MQTT
datasource plugin only works on that tag).
...
- `grafana/grafana:11.5.4` (**pinned — do not upgrade**) with `GF_INSTALL_PLUGINS="grafana-mqtt-datasource 1.3.3,yesoreyeram-infinity-datasource 3.11.1"` — a bad plugin version (`plugin.versionNotFound`) kills the container → Nginx 502
- `intel/dlstreamer:2026.1.0-ubuntu24` (one-shot in `install.sh`: model download + INT8 quantize + TLS cert)

## Layout (flat)

Generate a flat `{{STACK_DIR}}/` — top-level `README.md`, `docker-compose.yml`,
`.env`, `validate_env.sh`, `install.sh`, the `sample_*.sh`/`update_dashboard.sh`
scripts, a `src/` tree (`dlstreamer-pipeline-server/`, `mosquitto/`,
`node-red/`, `grafana/`, `nginx/`), and `tests/`. The **full annotated file
tree** is in [`references/INSTALL.md`](references/INSTALL.md).

### `README.md` (required content)

The generated `README.md` MUST document, at minimum:

- **Architecture** — data path `DLSPS → MQTT (Mosquitto) → Node-RED → Grafana`
  plus decoupled video `DLSPS WHIP → MediaMTX → browser WHEP (Grafana <iframe>)`,
  Coturn for ICE/TURN, all behind the Nginx TLS proxy. Include the reference
  ASCII diagram and the seven containers (`nginx, dlstreamer-pipeline-server,
...
- **Access URLs and credentials** — dashboard `https://<HOST_IP>/grafana/`,
  **default Grafana login `admin` / `admin`** (state explicitly; prompts to
  change on first login), DLSPS REST base `https://<HOST_IP>/api/pipelines`,
  WHEP player `https://<HOST_IP>/mediamtx/{{DETECTIONS_TOPIC_PREFIX}}_N/`.
- **Configuration** — key `.env` values (`HOST_IP`, GIDs, TURN creds) and how
  to swap the video source for RTSP.

## Template variable substitution

Every `{{VAR}}` in code blocks MUST be substituted with its concrete value
BEFORE writing the file — a literal `{{...}}` left in `nginx.conf`,
`config.json`, `flows.json`, the dashboard JSON, or a test file is a syntax
error.
...
templates.
- `dlstreamer-coding-agent` — pipeline JSON authoring (and the demo/PoC DL Streamer app when `{{MODE}}=demo`)
- `dlsps-user` (oep/skills) — DLSPS deploy/config/REST; default `{{MODE}}=production` path ([`references/PIPELINE.md`](references/PIPELINE.md))
- `model-download` (oep/edge-ai-libraries) — OMZ model IR
- `scenescape-setup` (oep/skills) — **only when `{{SCENESCAPE}}=yes`** ([`references/SCENESCAPE.md`](references/SCENESCAPE.md))
...
7. `curl --cacert src/nginx/ssl/server.crt https://localhost/mediamtx/{{DETECTIONS_TOPIC_PREFIX}}_1/`
   returns 200 (WHEP player HTML) once pipelines run; MediaMTX logs show the
   WHIP publisher connected for each `peer-id`.
...
9. Grafana at `https://localhost/grafana` (admin/admin) shows live
   ...
   panels (needs `GF_SECURITY_ALLOW_EMBEDDING=true`). The MQTT datasource
   health endpoint (`/grafana/api/datasources/uid/mqtt_ds/health`) MUST
   return `"MQTT Connected"` — broker address goes in **`jsonData.uri`**
   (`tcp://broker:1883`), NOT `url:`/`jsonData.host/port` (→ blank panels).
   Root redirect-loop → drop the trailing slash from `/grafana/` `proxy_pass`.
