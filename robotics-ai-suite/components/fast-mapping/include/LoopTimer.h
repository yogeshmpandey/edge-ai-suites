...
public:
    ...
    inline void finish()
    {
        ...
        // Format result here
        result_ = (boost::format(
                      "Totel elapsed time: %1$.2lf s  Frequency: %2$.1f Hz\n") %
                      (totalTimeMs / 1000) % (count_ * 1000 / totalTimeMs)).str();
    }
...
private:
    struct ProcInfo
    {
        ...
        inline void start(const timespec &time)
        {
            startTime_ = time;
            if (verbose_)
                printf("[LoopTimer] %s starts\n", name.c_str());
        }

        inline void end(const timespec &time)
        {
            long long timeNs = (time.tv_sec - startTime_.tv_sec) * 1000000000ULL +
                               time.tv_nsec - startTime_.tv_nsec;
            totalTimeNs += timeNs;
            count++;
            if (verbose_)
