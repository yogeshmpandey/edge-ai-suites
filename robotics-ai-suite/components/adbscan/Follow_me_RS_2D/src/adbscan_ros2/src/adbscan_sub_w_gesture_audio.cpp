...
class MinimalSubscriber : public rclcpp::Node
{
  public:
    MinimalSubscriber()
    : Node("adbscan_sub_w_gesture_audio_node")
    {
  ...
  private:
    ...
    void topic_2D_callback(const sensor_msgs::msg::LaserScan::SharedPtr _msg) 
    {
      ...
      if (result == 0) //function successful, run ADBSCAN to obtain a list of objects
      {
        ...
        if (new_target_loc.x  > 0 && (this->gesture_category_ == "Thumb_Up" || this->audio_category_ == "start"))
        {
          this->motor_initiated_ = true;
        }
        if (this->motor_initiated_ && new_target_loc.x > 0 && this->gesture_category_ != "Thumb_Down" && this->audio_category_ != "stop")
        {
    ...
    string audio_category_; // supported commands: "no_action", "start", "stop", "speed", "slow", "right", "left"
    ...
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr publisher_motor_state_; // this is subscribed by the text_to_speech node
