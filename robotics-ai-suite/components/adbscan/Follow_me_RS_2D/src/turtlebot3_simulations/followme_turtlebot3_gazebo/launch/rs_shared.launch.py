...
"""Terminal 1 — Gazebo world + bridges for RealSense follow-me.

Launches:
    • empty_world with two robots (TurtleBot3 waffle **depth model** + guide robot)
    • Gazebo-ROS bridges: clock, PointCloud2, cmd_vel (follower + guide),
      guide odometry, camera image (remapped from /camera/image to /camera/image_raw)

The custom turtlebot3_waffle_depth model has an RGBD camera that publishes
PointCloud2 on /camera/points — required by the RS ADBSCAN config.

Run rs_gesture.launch.py or rs_gesture_audio.launch.py in a second
...
def generate_launch_description():
    ...
    # Use the custom waffle model with RGBD (depth) camera.
    # The RGBD camera publishes PointCloud2 on /camera/points in Gazebo.
    depth_model_sdf = os.path.join(
        get_package_share_directory('followme_turtlebot3_gazebo'),
        'models',
        'turtlebot3_waffle_depth',
        'model.sdf',
    )

    # --- Gazebo world with two robots (depth-camera waffle + guide) ---
    launch_gz_world_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'model_sdf_path': depth_model_sdf,
        }.items(),
    )
    ...
    # RGBD camera PointCloud2 (Gazebo → ROS2)
    gz_pointcloud_bridge = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
    ...
    # The depth model publishes RGB on /camera/image (not /camera/image_raw),
    # so we remap to /camera/image_raw for consistent RViz config.
    ...
    ld.add_action(gz_pointcloud_bridge)
