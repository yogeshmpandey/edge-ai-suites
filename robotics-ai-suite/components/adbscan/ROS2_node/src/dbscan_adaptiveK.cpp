...
void get_active_map_using_oneapi_search_octree(LiDAR_data_3D_t *x, FPDS_t *esp_array, vector<int> *neighbors, INT32_t num_points) {
...
void get_active_map_using_oneapi_search_kdtree(LiDAR_data_3D_t *x, FPDS_t *esp_array, vector<int> *neighbors, INT32_t num_points){
	cout<<"Using get_active_map_using_oneapi_search_kdtree\n";
	//local
	...
	unordered_map<float, pair<pcl::PointCloud<pcl::PointXYZ>, vector<int>>> list_clouds;
	...
	for (auto it=list_clouds.begin(); it != list_clouds.end(); ++it){
		...
		s_kdtree_oneapi.radiusSearch(it->second.first, radius, k_indices, k_distances);
...
void dbscan_adaptiveK(
	...
	INT32_t *type, 
	INT32_t num_points,
	std::string oneapi_library_name)
{
	...
	std::string oneapi_library_name)
{
	...
	*/
	...
	else
	{
		...
		{
			std::cout << "Invalid parameter for oneapi_library: " << oneapi_library_name << ", Supported options: oneapi_kdtree | oneapi_octree | pcl_kdtree" << std::endl;
	...
	for (int i = 0; i < num_points; i++)
	{
		if (touched[i] == 0)
		{
			...
			}else if (length >= k + 1)
			{
				...
				{
					ind = active_q.front(); active_q.pop();
