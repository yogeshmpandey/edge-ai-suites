...
int main(int argc, char ** argv)
{
  try {
    ...
    model_names->for_each([&](auto && model_name_element) {
      ...
      if (!width.has_value()) {
        std::cout << "\033[1;31m"
                  << "Width is empty, please provide width"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      if (!height.has_value()) {
        std::cout << "\033[1;31m"
                  << "Height is empty, please provide height"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      if (task == "") {
        std::cout
          << "\033[1;31m"
          << "Task is empty, please provide one of the following: detection, segmentation, pose"
          << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      if (model_size == "") {
        std::cout << "\033[1;31m"
                  << "Model size is empty, please provide one of the following: n, s, m, l, xl"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      model_path =
        download_model(cv::Size(width.value(), height.value()), task_enum, half, model_size_enum);
    ...
    pipeline_names->for_each([&](auto && pipeline_name_element) {
      ...
      if (device == "") {
        device = "CPU";
        std::cout << "\033[1;33m"
                  << "Device is empty, using CPU"
                  << "\033[0m" << std::endl;
      }

      int64_t performance_mode =
        config[pipeline_name]["performance_mode"].value<int64_t>().value_or(1);
      ...
      if (num_requests == -1) {
        num_requests = 1;
        std::cout << "\033[1;33m"
                  << "Number of requests is empty, using 1"
                  << "\033[0m" << std::endl;
      }
      ...
      bool publish_detections =
        config[pipeline_name]["publish_detections"].as_boolean()->value_or(true);
      ...
      if (rgb_topic_max_fps_vector.size() != rgb_topic_vector.size()) {
        std::cout << "\033[1;31m"
                  << "rgb_topic and rgb_topic_max_fps need to be same length"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      if (rgbd_topic_max_fps_vector.size() != rgbd_topic_rgb_vector.size()) {
        std::cout << "\033[1;31m"
                  << "rgbd_topic_rgb and rgbd_topic_max_fps need to be same length"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      if (rgbd_topic_rgb_vector.size() != rgbd_topic_depth_vector.size()) {
        std::cout << "\033[1;31m"
                  << "rgbd_topic_rgb and rgbd_topic_depth need to be same length"
                  << "\033[0m" << std::endl;
        raise(SIGINT);
      }
      ...
      pipeline_ptr->configure(
        pipeline_name, models[model_name], device, performance_mode_enum, priority_enum,
        precision_enum, num_requests, workers, map_frame, queue_size, publish_video,
        publish_detections);
      ...
      for (size_t i = 0; i < rgbd_topic_rgb_vector.size(); i++) {
        pipeline_ptr->add_joint_rgb_depth_topics(
          rgbd_topic_rgb_vector[i], rgbd_topic_depth_vector[i], rgbd_topic_max_fps_vector[i]);
      }
    });
