...
equirectangular::equirectangular(const std::string& name, const color_order_t& color_order, const unsigned int cols,
...
equirectangular::equirectangular(const YAML::Node& yaml_node)
    : equirectangular(yaml_node["Camera.name"].as<std::string>(), load_color_order(yaml_node),
...
void equirectangular::undistort_keypoints(const std::vector<cv::KeyPoint>& dist_keypts,
...
void equirectangular::convert_bearings_to_keypoints(const eigen_alloc_vector<Vec3_t>& bearings,
                                                    std::vector<cv::KeyPoint>& undist_keypts) const
{
...
bool equirectangular::reproject_to_image(const Mat33_t& rot_cw, const Vec3_t& trans_cw, const Vec3_t& pos_w,
...
bool equirectangular::reproject_to_image_bound(const Mat33_t& rot_cw, const Vec3_t& trans_cw, const Vec3_t& pos_w,
                                         Vec2_t& reproj, float& x_right, [[maybe_unused]] float x_bound_expand,
...
bool equirectangular::reproject_to_bearing(const Mat33_t& rot_cw, const Vec3_t& trans_cw, const Vec3_t& pos_w,
                                           Vec3_t& reproj) const
{
    // convert to camera-coordinates
