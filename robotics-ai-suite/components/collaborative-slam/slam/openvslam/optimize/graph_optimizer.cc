                               ...
                               const std::map<data::keyframe*, std::set<data::keyframe*>>& loop_connections) const
{
    ...
    const auto insert_edge = [&optimizer, &vertices, &inserted_edge_pairs](KeyframeID id1, KeyframeID id2,
...

    {
        ...
        for (auto lm : all_lms) {
            ...
            const auto id = (lm->loop_fusion_identifier_ == curr_keyfrm->id_) ? lm->ref_keyfrm_id_in_loop_fusion_
