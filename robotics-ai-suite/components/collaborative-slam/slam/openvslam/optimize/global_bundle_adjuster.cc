...
global_bundle_adjuster::global_bundle_adjuster(const KeyframeID first_keyfrm_id_in_db, data::map_database* map_db,
                                               const unsigned int num_iter, const bool use_huber_kernel)
...
void global_bundle_adjuster::optimize(const KeyframeID lead_keyfrm_id_in_global_BA, bool* const force_stop_flag) const
{
    // 1. データを集める
    ...
    for (const auto keyfrm : keyfrms) {
        ...
        auto keyfrm_vtx = keyfrm_vtx_container.create_vertex(keyfrm, keyfrm->id_ == first_keyfrm_id_in_db_);
    ...
    for (unsigned int i = 0; i < lms.size(); ++i) {
        ...
        for (const auto& obs : observations) {
            ...
            auto reproj_edge_wrap =
                reproj_edge_wrapper(keyfrm, keyfrm_vtx, lm, lm_vtx, idx, undist_keypt.pt.x, undist_keypt.pt.y, x_right,
...
void global_bundle_adjuster::optimize(const KeyframeID lead_keyfrm_id_in_global_BA, bool* const force_stop_flag,
                                      const MapID map_id1, const MapID map_id2, const KeyframeID fixed_frame_id) const
{
    // 1. データを集める
    ...
    for (const auto keyfrm : keyfrms) {
        ...
        auto keyfrm_vtx = keyfrm_vtx_container.create_vertex(
            keyfrm, keyfrm->id_ == fixed_frame_id);  // Need to be modified for multi-slam, set the constant frame
        optimizer.addVertex(keyfrm_vtx);
    }
