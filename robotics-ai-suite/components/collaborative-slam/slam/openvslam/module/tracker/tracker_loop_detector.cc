...
bool tracker_loop_detector::detect_loop_candidates()
{
    ...
    // detection
    //    the number of each of the candidate keyframe sets that detected are counted every time when this member
    //    functions is called if the keyframe sets were detected at the previous call, is is contained in
    //    `cont_detected_keyfrm_sets_` (note that "match of two keyframe sets" means the intersection of the two sets is
    //    NOT empty)
    ...
    std::cout << "loop_candidates_to_validate_size: " << loop_candidates_to_validate_.size() << std::endl;
...
bool tracker_loop_detector::validate_candidates()
{
    ...
    const bool candidate_is_found = select_loop_candidate_via_Sim3(
        loop_candidates_to_validate_, selected_candidate_, g2o_Sim3_world_to_curr_, curr_match_lms_observed_in_cand_);
    ...
    // matches between the keypoints in the current and the landmarks observed in the covisibilities of the selected
    // candidate
...
bool tracker_loop_detector::select_loop_candidate_via_Sim3(const std::vector<data::keyframe*>& loop_candidates,
