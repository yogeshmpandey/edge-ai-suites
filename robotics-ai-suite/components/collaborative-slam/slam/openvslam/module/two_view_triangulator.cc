...
bool two_view_triangulator::triangulate(const unsigned idx_1, const unsigned int idx_2, Vec3_t& pos_w) const
{
    ...
    const auto cos_stereo_parallax_1 =
        is_stereo_1 ? std::cos(2.0 * atan2(camera_1_->true_baseline_ / 2.0, keyfrm_1_->depths_.at(idx_1))) : 2.0;
    const auto cos_stereo_parallax_2 =
        is_stereo_2 ? std::cos(2.0 * atan2(camera_2_->true_baseline_ / 2.0, keyfrm_2_->depths_.at(idx_2))) : 2.0;
    ...
    if (!check_scale_factors(pos_w, keyfrm_1_->scale_factors_.at(keypt_1.octave),
...
template <typename T>
bool two_view_triangulator::check_reprojection_error(const Vec3_t& pos_w, const Mat33_t& rot_cw, const Vec3_t& trans_cw,
                                                     camera::base* const camera, const cv::Point_<T>& keypt,
                                                     const float x_right, const float sigma_sq,
