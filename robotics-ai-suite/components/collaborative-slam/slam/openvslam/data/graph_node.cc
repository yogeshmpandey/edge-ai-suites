...
void graph_node::recover_spanning_connections()
{
    ...
    if (!spanning_parent_) {
        ...
        for (auto& spanning_child : spanning_children_) {
            if (!spanning_child) continue;
            if (spanning_child->will_be_erased()) {
                continue;
            }
            spanning_child->graph_node_->set_spanning_parent(nullptr);
            spdlog::debug("set spanning parent of its spanning child {} to nullptr!", spanning_child->id_);
        }
    ...
    while (!spanning_children_.empty()) {
        ...
        if (max_is_found) {
            ...
            spdlog::debug("Case 1: keyframe {} changes spanning parent from {} to {}!", max_weight_child->id_,
    ...
    if (!spanning_children_.empty()) {
        ...
        for (auto& spanning_child : spanning_children_) {
            spdlog::debug("Case 2: keyframe {} changes spanning parent from {} to {}!", spanning_child->id_,
...
void graph_node::erase_loop_edge(keyframe* keyfrm)
{
    ...
    if (keyfrm->graph_node_->has_loop_edge()) {
        spdlog::warn("must first clear the loop_edges_ of {} before erasing it from loop_edges of {}",
                     keyfrm->id_, owner_keyfrm_->id_);
        return;
    }
    ...
    for (const auto& loop_keyfrm : loop_edges_copy) {
        ...
        */
        if (owner_keyfrm_ != loop_keyfrm) {
            spdlog::debug("keyframe {}: erase itself from the loop_edges of keyframe {}", owner_keyfrm_->id_,
                          loop_keyfrm->id_);
...
void graph_node::add_constraint_edge(int constraint_id, keyframe* keyfrm)
{
    ...
    if (front_rear_camera_constraint_.first < 0 || !front_rear_camera_constraint_.second ||
