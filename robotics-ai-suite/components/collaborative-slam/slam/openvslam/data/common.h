...
inline bool get_cell_indices(camera::base* camera, const cv::KeyPoint& keypt, int& cell_idx_x, int& cell_idx_y)
{
    ...
    return (0 <= cell_idx_x && cell_idx_x < static_cast<int>(camera->num_grid_cols_) && 0 <= cell_idx_y &&
...
std::vector<unsigned int> get_keypoints_in_cell(
    ...
    const float ref_y, const float margin, const int min_level = -1, const int max_level = -1);
