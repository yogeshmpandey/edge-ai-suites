...
nlohmann::json convert_translation_to_json(const Vec3_t& trans_cw) { return {trans_cw(0), trans_cw(1), trans_cw(2)}; }
...
std::vector<cv::KeyPoint> convert_json_to_keypoints(const nlohmann::json& json_keypts)
{
    ...
    for (unsigned int idx = 0; idx < json_keypts.size(); ++idx) {
        ...
        keypts.at(idx) =
            cv::KeyPoint(json_keypt.at("pt").at(0).get<float>(), json_keypt.at("pt").at(1).get<float>(), 0,
...
std::vector<unsigned int> get_keypoints_in_cell(
    ...
    const std::vector<std::vector<std::vector<unsigned int>>>& keypt_indices_in_cells, const float ref_x,
    const float ref_y, const float margin, const int min_level, const int max_level)
{
