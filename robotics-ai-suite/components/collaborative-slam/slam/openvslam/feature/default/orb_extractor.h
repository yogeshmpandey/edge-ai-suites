...
private:
    ...
    std::list<orb_extractor_node<cv::KeyPoint>> initialize_nodes(const std::vector<cv::KeyPoint>& keypts_to_distribute,
                                                   const int min_x, const int max_x, const int min_y, const int max_y) const;

    //! Assign child nodes to the all node list
    void assign_child_nodes(const std::array<orb_extractor_node<cv::KeyPoint>, 4>& child_nodes, std::list<orb_extractor_node<cv::KeyPoint>>& nodes,
                            std::vector<std::pair<int, orb_extractor_node<cv::KeyPoint>*>>& leaf_nodes) const;

    //! Find keypoint which has maximum value of response
    std::vector<cv::KeyPoint> find_keypoints_with_max_response(std::list<orb_extractor_node<cv::KeyPoint>>& nodes) const;
