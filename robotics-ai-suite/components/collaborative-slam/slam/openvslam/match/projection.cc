...
unsigned int projection::match_frame_and_landmarks(data::frame& frm,
                                                   const std::vector<data::landmark*>& local_landmarks,
                                                   const float original_margin) const
{
    ...
    for (auto local_lm : local_landmarks) {
        ...
        const auto indices_in_cell = frm.get_keypoints_in_cell(
            ...
            margin * frm.scale_factors_.at(pred_scale_level), start_predict_scale_level, end_predict_scale_level);
        ...
        if (!lm_desc.ptr<uint32_t>()) {
            spdlog::debug("invalid descriptor of this landmark: {}  from server : {}", local_lm->id_,
                                                  ...
                                                  const float margin, const unsigned int hamm_dist_thr) const
{
    ...
    for (unsigned int idx = 0; idx < landmarks.size(); idx++) {
        ...
        const auto indices =
            curr_frm.get_keypoints_in_cell(reproj(0), reproj(1), margin * curr_frm.scale_factors_.at(pred_scale_level),
                                           min_level, max_level);
                                                 ...
                                                 const float margin) const
{
    ...
    for (auto lm : landmarks) {
        ...
        const auto indices =
            keyfrm->get_keypoints_in_cell(reproj(0), reproj(1), margin * keyfrm->scale_factors_.at(pred_scale_level));
                                                  ...
                                                  const float margin) const
{
    ...
    if (s_12 == 0.0f) throw std::runtime_error("s_12 must be non-zero");
