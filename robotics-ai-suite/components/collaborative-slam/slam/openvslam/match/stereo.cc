...
stereo::stereo(const std::vector<cv::Mat>& left_image_pyramid, const std::vector<cv::Mat>& right_image_pyramid,
               ...
               const std::vector<float>& inv_scale_factors, const float focal_x_baseline, const float true_baseline)
...
#pragma omp parallel for
    ...
    for (unsigned int idx_left = 0; idx_left < num_keypts_; ++idx_left) {
        ...
        find_closest_keypoints_in_stereo(idx_left, scale_level_left, candidate_indices_right, min_x_right, max_x_right,
                                         best_idx_right, best_hamm_dist);
        // ハミング距離が閾値を満たさなければ破棄
        ...
        const auto is_valid =
            compute_subpixel_disparity(keypt_left, keypt_right, best_x_right, best_disp, best_correlation);
        // 見つからなければ破棄
        if (!is_valid) {
            continue;
        }
                                        ...
                                        float& best_x_right, float& best_disp, float& best_correlation) const
{
    ...
    for (int offset = -slide_width; offset <= +slide_width; ++offset) {
        ...
        patch_right -=
            patch_right.at<float>(win_size, win_size) * cv::Mat::ones(patch_right.rows, patch_right.cols, CV_32F);
