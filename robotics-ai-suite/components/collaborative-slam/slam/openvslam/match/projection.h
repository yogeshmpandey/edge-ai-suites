...
public:
    ...
    unsigned int match_frame_and_landmarks(data::frame& frm, const std::vector<data::landmark*>& local_landmarks,
                                           const float margin = 5.0) const;

    unsigned int match_frame_and_server_landmarks(data::frame& frm,
                                                  const std::vector<data::landmark*>& server_landmarks,
                                                  const float margin) const;

    unsigned int match_frame_and_local_landmarks(data::frame& frm, const std::vector<data::landmark*>& local_landmarks,
                                                 const float margin) const;

    //! last frameで観測している3次元点をcurrent frameに再投影し，frame.landmarks_に対応情報を記録する
    unsigned int match_current_and_last_frames(data::frame& curr_frm, const data::frame& last_frm,
                                               const float margin) const;
