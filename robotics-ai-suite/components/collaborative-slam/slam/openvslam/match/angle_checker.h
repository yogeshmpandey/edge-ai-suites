...
angle_checker<T>::angle_checker(const unsigned int histogram_length, const unsigned int num_bins_thr)
    : histogram_length_(histogram_length), inv_histogram_length_(1.0f / histogram_length), num_bins_thr_(num_bins_thr)
{
...
inline void angle_checker<T>::append_delta_angle(float delta_angle, const T& match)
{
    ...
    if (bin >= histogram_length_) {
        spdlog::warn("{}: beyond the boundary of angle_histogram_ vector, size: {}", __func__, histogram_length_);
        return;
    }
...
std::vector<T> angle_checker<T>::get_valid_matches() const
{
    ...
    for (unsigned int bin = 0; bin < histogram_length_; ++bin) {
        const bool is_valid_match =
            std::any_of(bins.begin(), bins.begin() + num_bins_thr_, [bin](const unsigned int i) { return bin == i; });
        if (is_valid_match) {
...
std::vector<unsigned int> angle_checker<T>::index_sort_by_size(const std::vector<std::vector<T>>& vec) const
{
    ...
    std::sort(indices.begin(), indices.end(),
              [&vec](const unsigned int a, const unsigned int b) { return (vec.at(a).size() > vec.at(b).size()); });
    return indices;
}
