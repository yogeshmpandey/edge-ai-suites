...
void pnp_solver::find_via_ransac(const unsigned int max_num_iter, const bool recompute) {
    ...
    for (unsigned int iter = 0; iter < max_num_iter; ++iter) {
        ...
        // This also avoids the situation of num_correspondences_ == 0, which will cause devision-by-zero in compute_pose
        if (num_correspondences_ < min_set_size) continue;
    ...
    // Above we've made sure num_correspondences_ == min_set_size for every candidate of (best_rot_cw_, best_trans_cw_)
    // And exit the function if there is no valid solution
    // So we can safely assume the following, and devision-by-zero not happen in the next compute_pose
...
void pnp_solver::choose_control_points() {
    ...
    if (num_correspondences_ == 0) throw std::runtime_error("num_correspondences_ must be non-zero");
