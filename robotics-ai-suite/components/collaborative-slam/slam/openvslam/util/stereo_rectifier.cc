...
stereo_rectifier::stereo_rectifier(camera::base* camera, const cv::Mat& K_l, const cv::Mat& D_l, const cv::Mat& R_l,
...
void stereo_rectifier::init_undist_map(camera::base* camera, const cv::Mat& K_l, const cv::Mat& D_l, const cv::Mat& R_l,
                                       const cv::Mat& K_r, const cv::Mat& D_r, const cv::Mat& R_r) {
    // set image size
...
cv::Mat stereo_rectifier::parse_vector_as_mat(const cv::Size& shape, const std::vector<double>& vec) {
    ...
    if (shape_data_size != vec.size()) throw std::runtime_error("Invalid stereo rectifier parameter!");
