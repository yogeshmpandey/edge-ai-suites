...
class StereoInertialBenchmark:
    ...
    def run_single_test(self):
        ...
        launch_params = ["camera_setup:=Stereo_Inertial", "gui:=false", "rviz:=false", "publish_tf:=false",
                        "log_level:=warning"]
        if self.dataset == "euroc":
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description="Automatically run EuRoC dataset with stereo and imu input and print the accuracy data to standard output")
...
def generate_test_list(bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict):
    ...
    for name in groundtruth_list:
        ...
        item = StereoInertialBenchmark(bagfile_prefix_list[bagfile_index], bagfile, groundtruth, args_dict["ROS1PATH"], args_dict["DATASET"], args_dict["WAIT"], args_dict["RESULT"])
