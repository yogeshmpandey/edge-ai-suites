...
class StereoBenchmark:
    ...
    def __get_sequence_number(self):
        ...
        self.sequence = int(seq[len(seq) - 2:])
    ...
    def run_single_test(self):
        ...
        if self.dataset == "euroc":
            ...
            seq_cmd = seq + ":=true"
            launch_file_cmd = ["ros2", "launch", "univloc_tracker", "kitti_stereo.launch.py", seq_cmd]
        launch_params = ["camera_setup:=Stereo", "gui:=false", "rviz:=false", "publish_tf:=false", "log_level:=warning"]
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description="Automatically run EuRoC dataset with stereo input and print the accuracy data to standard output")
    parser.add_argument("dataset", type=str, help="The dataset to be tested, supported options: euroc and kitti",
                        choices=["euroc", "kitti"])
    ...
    parser.add_argument("-l", "--bagfile_list", type=str, dest="bagfile_list",
                        help="The list specifies which dataset need to be tested", default=default_bagfile_dir)
    ...
    parser.add_argument("--output", action="store_true",
                        help="Output accuracy data for each test loop.")
...
def generate_test_list(bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict):
    ...
    for name in groundtruth_list:
        ...
        item = StereoBenchmark(bagfile_prefix_list[bagfile_index], bagfile, groundtruth, args_dict["ROS1PATH"], args_dict["DATASET"], args_dict["WAIT"], args_dict["RESULT"])
...
def main():
    ...
    if args_dict["DATASET"] == "euroc":
        groundtruth_list = generate_groundtruth_list_euroc(args_dict["GROUNDTRUTH"], bagfile_prefix_list)
    elif args_dict["DATASET"] == "kitti":
        groundtruth_list = generate_groundtruth_list_kitti(args_dict["GROUNDTRUTH"], bagfile_prefix_list)
