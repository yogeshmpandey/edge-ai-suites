...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description="Automatically run TUM dataset with RGBD input and print the accuracy data to standard output")
    parser.add_argument("dataset", type=str, help="The dataset to be tested, option: tum",
                        choices=["tum"], default="./")
    ...
    if os.path.isdir(args.groundtruth_dir):
        ...
        print("Since the directory of groundtruth files is not a directory, will use the directory of bag files instead")
...
def generate_test_list(bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict):
    ...
    for name in groundtruth_list:
        ...
        item = TrajectoryBenchmark(bagfile_prefix_list[bagfile_index], bagfile, groundtruth, args_dict["ROS1PATH"], args_dict["DATASET"], args_dict["WAIT"], args_dict["RESULT"])
...
def main():
    ...
    for item in benchmark_list:
        ...
        for test_loop in range(args_dict["REPEAT"]):
            ...
            if status != 0:
                ret = -1
                print('The result evaluation of test "{}" is failed'.format(item.name), file=sys.stderr)
                continue
