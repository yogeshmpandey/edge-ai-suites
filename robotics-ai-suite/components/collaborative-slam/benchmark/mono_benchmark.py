...
class MonocularBenchmark:
    ...
    def run_single_test(self):
        ...
        if self.dataset == "euroc":
            ...
            seq_cmd = seq + ":=true"
            launch_file_cmd = ["ros2", "launch", "univloc_tracker", "kitti_mono.launch.py", seq_cmd]
        launch_params = ["camera_setup:=Monocular", "gui:=false", "rviz:=false", "publish_tf:=false", "log_level:=warning"]
...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description="Automatically run TUM/EuRoC dataset with monocular input and print the accuracy data to standard output")
    parser.add_argument("dataset", type=str, help="The dataset to be tested, option: tum/euroc/kitti",
...
def generate_test_list(bagfile_list, bagfile_prefix_list, groundtruth_list, args_dict):
    ...
    for name in groundtruth_list:
        ...
        item = MonocularBenchmark(bagfile_prefix_list[bagfile_index], bagfile, groundtruth, args_dict["ROS1PATH"], args_dict["DATASET"], args_dict["WAIT"], args_dict["RESULT"])
...
def main():
    ...
    if args_dict["DATASET"] == "tum":
        ...
        groundtruth_list = generate_groundtruth_list(args_dict["GROUNDTRUTH"], bagfile_prefix_list, False)
