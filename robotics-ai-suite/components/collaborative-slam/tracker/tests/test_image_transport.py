...
def args_parse(default_bagfile_dir):
    parser = argparse.ArgumentParser(description="Automatically test tracker image transport feature")
    ...
    parser.add_argument("-r", "--raw_transport", type=str, dest="raw_transport",
                        help="Whether to use raw image transport", default="true", choices=["true", "false"])
    parser.add_argument("-t", "--pose_threshold", type=int, dest="pose_threshold",
    ...
    args_dict.update({"RAW": args.raw_transport})
    args_dict.update({"THRESHOLD": args.pose_threshold})
...
def generate_launch_description(enable_raw_transport):
    ld = LaunchDescription([
        ...
        DeclareLaunchArgument(
            name='raw_transport',
            default_value=enable_raw_transport
        ),
        DeclareLaunchArgument(
            name='camera_info_topic',
            default_value='data_throttled_camera_info'
        ),
        DeclareLaunchArgument(
            name='image_topic',
            default_value='data_throttled_image'
        ),
        DeclareLaunchArgument(
            name='depth_topic',
            default_value='data_throttled_image_depth'
        ),
        DeclareLaunchArgument(
            name='image_frame',
