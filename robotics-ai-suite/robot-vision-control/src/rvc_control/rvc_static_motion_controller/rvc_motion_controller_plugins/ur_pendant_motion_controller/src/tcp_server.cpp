...
// SPDX-FileCopyrightText: (C) 2021 FZI Forschungszentrum Informatik
...
#include "ur_pendant_motion_controller/tcp_server.hpp"
...
TCPServer::~TCPServer()
{
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Destroying TCPServer object.");
  shutdown();
  close(listen_fd_);
}
...
void TCPServer::init()
{
  ...
  int flag = 1;
  int ret = setsockopt(listen_fd_, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int));

  if (ret < 0 )
  {
    RCLCPP_ERROR(rclcpp::get_logger("PP"),"setsockopt ret %d", ret);
  }

  ret = setsockopt(listen_fd_, SOL_SOCKET, SO_KEEPALIVE, &flag, sizeof(int));
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Created socket with FD %d", (int)listen_fd_);
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Created read pipe at FD %d", self_pipe_[0]);
...
void TCPServer::shutdown()
{
  ...
  {
    worker_thread_.join();
    RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Worker thread joined.");
  }
}
...
void TCPServer::bind()
{
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Bound %d:%d to FD %d", server_addr.sin_addr.s_addr, port_, (int)listen_fd_);
...
void TCPServer::startListen()
{
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Listening on port %d", port_);
}
...
void TCPServer::handleConnect()
{
  ...
  {
    RCLCPP_WARN(rclcpp::get_logger("PP"),"Connection attempt on port %d while maximum number of clients (%d) is already connected. Closing "
                  "connection.",
...
void TCPServer::spin()
{
  ...
  if (sel < 0)
  {
    RCLCPP_ERROR(rclcpp::get_logger("PP"),"select() failed. Shutting down socket event handler.");
    keep_running_ = false;
    return;
  }
  ...
  {
    RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Activity on self-pipe");
    char buffer;
    if (read(self_pipe_[0], &buffer, 1) == -1)
    {
      while (true)
      {
        if (errno == EAGAIN)
          ...
          RCLCPP_ERROR(rclcpp::get_logger("PP"),"read failed");
      }
    }
    else
    {
      RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Self-pipe triggered");
      return;
    }
  }
  ...
  for (int i = 0; i <= maxfd_; i++)
  {
    ...
    {
      RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Activity on FD %d", i);
      if (listen_fd_ == i)
      {
...
{
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"%d disconnected.", fd);
  close(fd);
...
void TCPServer::worker()
{
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Finished worker thread of TCPServer");
}

void TCPServer::start()
{
  RCLCPP_DEBUG(rclcpp::get_logger("PP"),"Starting worker thread");
  keep_running_ = true;
