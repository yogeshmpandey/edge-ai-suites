...
class FastSAMPrompt:
    ...
    def __init__(self, source, results, device="cuda") -> None:
        ...
        #     import clip
        logger.info(f"Available clip model: {clip.available_models()}")
        self.clip = clip
...
#        return black_image
        image_array = np.array(image)
        x1, y1, x2, y2 = bbox
        extracted_image_array = image_array[y1:y2, x1:x2]
        ...
        resized_image = cv2.resize(extracted_image_array, (new_width, new_height),interpolation=cv2.INTER_AREA)
        final_image = np.ones((target_height,target_width, 3), dtype=np.uint8) * 255
        final_image[0:new_height, 0:new_width] = resized_image
        segmented_image = Image.fromarray(final_image)
    ...
    def _get_bbox_from_mask(mask):
        ...
        x2, y2 = x2 + int(margin/2), y2 + int(margin/2)
        x1, y1 = x1 - int(margin/2), y1 - int(margin/2)
    ...
    # ViT-L/14 yield better result but needs optimization to offload to GPU, ViT-B/32  ViT-L/14
    def text_prompt(self, text, model="ViT-B/32", threshold=0.65):
        """Processes a text prompt, applies it to existing results and returns the updated results."""
        confidence_score = 0.0
        if self.results[0].masks is not None:
            format_results = self._format_results(self.results[0], 0)
            cropped_boxes, cropped_images, not_crop, filter_id, annotations = self._crop_image(format_results)
            clip_model, preprocess = self.clip.load(model, device=self.device)
            scores = self.retrieve(clip_model, preprocess, cropped_boxes, text, device=self.device)
            max_idx = scores.argsort()
            max_idx = max_idx[-1]
            max_idx += sum(np.array(filter_id) <= int(max_idx))
            logger.info(f"Text prompt confidence score: {scores[max_idx]:.2}, model: {model}")
            confidence_score = np.array(scores[max_idx])
            if np.array(scores[max_idx]) <= threshold:
                self.results[0].masks = None
            else:
                self.results[0].masks.data = torch.tensor(np.array([annotations[max_idx]["segmentation"]]))
        return self.results, confidence_score
