...
class ContextualParaformer(Paraformer):
    ...
    def __init__(self, model_dir: Union[str, Path] = None,
        ...
        if not Path(model_dir).exists():
            print("[error] the ASR model not exist")
            # try:
            #     from modelscope.hub.snapshot_download import snapshot_download
            # except:
            #     raise "You are exporting model from modelscope, please install modelscope and try it again. To install modelscope, you could:\n" \
        ...
        if self.use_ov:
            if quantize:
                model_bb_file = os.path.join(model_dir, 'model_bb_fp16.xml')
                model_eb_file = os.path.join(model_dir, 'model_eb_fp16.xml')
                print(f"use_ov model dir: { model_dir } ")
            else:
                model_bb_file = os.path.join(model_dir, 'model_bb_fp32.xml')
                model_eb_file = os.path.join(model_dir, 'model_eb_fp32.xml')
                print(f"without_ov model dir: { model_dir } ")
        else:
            if quantize:
        ...
        if self.use_ov:
            ...
            model_config = {}
            #  if quantize:
            #     model_config["INFERENCE_PRECISION_HINT"] = "f16"
            #model_bb_file = "/data/xkd_data/8-SourceCode/HP_ASR/FunASR_0509/FunASR/model_bb.xml"
            #model_eb_file = "/root/.cache/modelscope/hub/iic/speech_seaco_paraformer_large_asr_nat-zh-cn-16k-common-vocab8404-pytorch/tmp/model_eb.xml"
            print("== model_bb_file == :", model_bb_file)
            print("== model_eb_file == :", model_eb_file)
            print("== model_config == :", model_config)
            device="CPU"
            self.ort_infer_bb = core.compile_model(model_bb_file, device_name=device, config=model_config)
            device="CPU"
            ov_eb_model = core.read_model(model_eb_file)
            self.ort_infer_eb = core.compile_model(ov_eb_model, device_name=device, config=model_config)
            # self.ort_infer_eb = core.compile_model(model_eb_file, device_name=device, config=model_config)
        else:
    ...
    def __call__(self, 
        ...
        if self.use_ov:
            ...
            # print("============ embedding model input ===========")
            # print("== hotwords ==",hotwords)
            # print("== hotwords_length ==",hotwords_length)
            bias_embed = bias_embed['hw_embed']
        else:
            [bias_embed] = self.eb_infer(hotwords, hotwords_length)
    ...
    def bb_infer(self, feats: np.ndarray,
        ...
        # print("============ backbound model input ===========")
        # print("== feats ==",feats)
        # print("== feats_len ==",feats_len)
        # print("== bias_embed ==",bias_embed)
        # tmp_input = {"speech":feats,
        #              "speech_lengths":feats_len,
        #              "bias_embed":bias_embed}
        # bb_input_path = "./bb_input.npy"
        # with open(bb_input_path, "a") as bb_f:
        #     bb_f.write(str(tmp_input))
        return outputs

    def eb_padding_for_npu(self, inputs, inputs_shape):
        resized_arr = np.zeros(inputs_shape, dtype=inputs.dtype)
        # 计算原始数组和目标尺寸的最小边界
        rows = min(inputs.shape[0], inputs_shape[0])
        cols = min(inputs.shape[1], inputs_shape[1])
        # 将原始数组的数据复制到新数组的相应位置
        resized_arr[:rows, :cols] = inputs[:rows, :cols]
        return resized_arr
            
    def eb_infer(self, hotwords, hotwords_length):
        if self.use_ov:
            # print(hotwords.astype(np.int32).shape)
            if self.ort_infer_eb.inputs[0].partial_shape.is_static:
                # print("model is static input shape, padding.............")
                inputs_shape = self.ort_infer_eb.inputs[0].shape
                hotwords = self.eb_padding_for_npu(hotwords, inputs_shape)
