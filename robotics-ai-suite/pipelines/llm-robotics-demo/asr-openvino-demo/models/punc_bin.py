...
class CT_Transformer():
    ...
    def __init__(self, model_dir: Union[str, Path] = None,
        ...
        if not Path(model_dir).exists():
            print("[error] the punc model not exist")
            # try:
        ...
        if self.use_ov:
            ...
            import time
            s= time.time()
            self.ort_infer = core.compile_model(model_file, device_name=device, config=model_config)
            print('punc model compile cost %.2f s'%(time.time()-s))
        else:
