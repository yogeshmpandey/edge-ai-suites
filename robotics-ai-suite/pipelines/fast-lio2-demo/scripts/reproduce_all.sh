#!/usr/bin/env bash
# One-command reproduce: apply Intel patches, build, check the UrbanLoco
# sequence configured in env.sh is present (see fetch_ulhk.sh for manual-
# download instructions if not), convert it to a ROS 2 bag (once), set up
